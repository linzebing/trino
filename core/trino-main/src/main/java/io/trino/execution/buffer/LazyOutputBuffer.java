            ...
            synchronized (this) {
                ...
                if (outputBuffer == null) {
                    ...
                    switch (newOutputBuffers.getType()) {
                        case PARTITIONED:
                            outputBuffer = new PartitionedOutputBuffer(taskInstanceId, stateMachine, newOutputBuffers, maxBufferSize, systemMemoryContextSupplier, executor);
                            break;
                        case BROADCAST:
                            outputBuffer = new BroadcastOutputBuffer(taskInstanceId, stateMachine, maxBroadcastBufferSize, systemMemoryContextSupplier, executor, notifyStatusChanged);
                            break;
                        case ARBITRARY:
                            outputBuffer = new ArbitraryOutputBuffer(taskInstanceId, stateMachine, maxBufferSize, systemMemoryContextSupplier, executor);
                            break;
                        case SPOOL:
                            ...
                            outputBuffer = new SpoolingExchangeOutputBuffer(stateMachine, newOutputBuffers, exchangeSink, systemMemoryContextSupplier);
                            break;
                        default:
